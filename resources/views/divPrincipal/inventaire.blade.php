    ...
    <div class="flex flex-wrap">
        <div id="myItems" class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-2">
            @foreach ($inventory as $item)
                <div id="{{ $item->item->item_id }}" class="card my-3 mx-3 p-3 bg-white rounded-lg shadow-md flex flex-col items-center justify-center">
                    <div class="flex flex-col items-center justify-center w-full max-w-sm mx-auto m-3">
                        <img src="img/img.png" class="w-32 h-32 object-cover object-center rounded-full">
                        <h2 class="text-lg text-gray-900 font-medium title-font my-4">{{ $item->item->name }}</h2>
                        <h3 class="text-gray-500 text-sm mb-4">x{{ $item->quantity }}</h3>
                    </div>
                </div>
            @endforeach
        </div>
