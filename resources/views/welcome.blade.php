...
@section('content')
    ...
    <aside id="default-sidebar"
        ...
        <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 light:bg-gray-800">
            ...
            <ul class="p-2 space-y-2 font-medium" id="playerMenu">
                ...
                <a id="menuSave" href="/logout" onclick="menuNavigation(this)" name="menuTab"
                    class="flex items-center px-4 py-2.5 text-sm font-medium transition-all duration-200 cursor-pointer rounded-lg group hover:text-white hover:bg-stone-600">
                ...
                <div class="flex items-center gap-x-2">
                    ...
                    <img class="object-cover w-12 h-12 rounded-lg"
                        src=" @php $playerSession = json_decode(session('player'))->avatar->image; echo $playerSession; @endphp"
                        alt="avatar">
                    ...
                    <div id="infoPlayer">
                        ...
                        <div class="w-full bg-gray-300 h-4 rounded">
                            <div class="h-full text-center text-xs text-white bg-green-500 rounded"
                                style="width: {{ (session('player')->xp / session('player')->xp_max) * 100 }}%">
                        ...
                        <h3 class="font-semibold text-gray-700 capitalize">
                            ...
                            <span id="hp_money">
                                {{ formatPrice(session('player')->money) }}
                            </span>
                        </h3>
                        <h3 class="font-semibold text-gray-700 capitalize">
                            <i class="fas fa-gem text-blue-600 "></i>
                            <span id="hp_gem">
                                {{ formatPrice(session('player')->gem) }}
                            </span>
                        </h3>
                        <h3 class="font-semibold text-gray-700 capitalize">
                            <i class="fas fa-bolt-lightning text-yellow-400"></i>
                            <span id="hp_nrj">
                                {{ formatPrice(session('player')->stamina) }}
                            </span>
                        </h3>
                    </div>
                </div>
    ...
    <div class="p-4 sm:ml-64">
        <div id="divPrincipal" class="p-4 border-2 border-dashed rounded-lg border-gray-200">
            ...
            {{-- Shop --}}
            <div name="divPrincipal" id="divShop" style="display: none" class="flex rounded-lg min-h-screen">
            ...
            {{-- Invocation --}}
            <div name="divPrincipal" id="divInvocation" style="display: none" class="flex rounded-lg min-h-screen">
                @include('divPrincipal.invocation')
            </div>
    ...
    @section('scripts')
        {{-- <script>
            ...
            window.addEventListener("load", setCapture(@json($captured)));
            window.addEventListener("load", setTeam(@json($team)));
